 ...
 */ 
function add(str) {
	document.smsForm.strData.focus();
	document.smsForm.strData.value+=str; 
	ChkLen();
	return;
}
...
function ChkLen() {
	...
	var pos;
	var msglen = 0;
	var len = smsForm.strData.value.length;
	for (i=0;i<len;i++){
		pos = smsForm.strData.value.charAt(i);
    ...
    if(isNewSubject && msglen <= 30) smsForm.strSubject.value = '';

    smsForm.strDataCount.value = msglen;
 ...
 */ 
function addItem() {
	if (smsForm.addCallNum.value == "" ) {
		alert("받는분 번호를 정확히 입력하시고 추가해주세요.");
		smsForm.addCallNum.value = "";
		smsForm.addCallNum.focus();
		return;
	} 

	if (!isInteger(smsForm.addCallNum.value, 1)) {
	...
	var i;
	rcvPhnId	= smsForm.addCallNum.value;
...
function addSelectBox(name, tel) {
	...
	index = smsForm.strDest.options.length;
	...
	if(phoneRe.exec(tel)){
		...
		for (i=0; i < index ; i++) {
			if (smsForm.strDest.options[i].text == name + "/" + tel + "/") {
				alert(name+"님은 이미 수신자에 포함되어 있습니다.");
				return;
			}
		}

		smsForm.strDest.options.length = index+1;
		smsForm.strDest.options[index].value =  tel +"/";
		smsForm.strDest.options[index].text = name + "/" + tel +"/";
		smsForm.strDest.options.length = index+1;
	} 
}   
...
function CWCheck(form) {
	...
	if (document.smsForm.chkSendFlag[0].checked) {	// 즉시전송을 선택했을 경우
		
		sDest.style.display = "none";
		document.smsForm.R_YEAR.disabled = true;
		document.smsForm.R_MONTH.disabled = true;
		document.smsForm.R_DAY.disabled = true;
		document.smsForm.R_HOUR.disabled = true;
		document.smsForm.R_MIN.disabled = true;
	}
	else											// 예약전송을 선택했을 경우
	{
		sDest.style.display = "block";	
		document.smsForm.R_YEAR.disabled = false;
		document.smsForm.R_MONTH.disabled = false;
		document.smsForm.R_DAY.disabled = false;
		document.smsForm.R_HOUR.disabled = false;
		document.smsForm.R_MIN.disabled = false;
		document.smsForm.R_YEAR.focus();
	}
	document.smsForm.strTelList.value = "";
}
...
function varcheck() {
	...
	var varStrData = document.smsForm.strData.value;
	var varStrSubject = document.smsForm.strSubject.value;
	var varStrCallBack = document.smsForm.strCallBack.value;
    ...
    if (document.getElementById("divLmsTitle").style.display != "none") {
        if(varStrSubject.length == 0) {
            document.smsForm.strSubjectrData.value = "[제목없음]";
        } else if (varStrSubject.length > 30) {
	        alert("LMS 제목은 30byte를 넘을 수 없습니다.");
            document.smsForm.strSubjectrData.focus();
            return false;
        }
    }

	// 받는 사람의 전화번호가 들어가 있는지 확인합니다.
	if (document.smsForm.strDest.length < 2){
		alert("수신자 번호를 입력해 주세요");
		document.smsForm.addCallNum.focus();
		return false;
	}
	...
	if (varStrCallBack  == null || varStrCallBack == ""){
		alert("회신번호를 입력하세요");
		document.smsForm.strCallBack.focus();
		return false;
	}
	...
	// 예약 년도가 정확한지 확인합니다.
	if (document.smsForm.chkSendFlag[1].checked && document.smsForm.R_YEAR.value.length!=4){
		alert("년도가 정확하지 않습니다.");
		document.smsForm.R_YEAR.focus();
		return false;
	}
	...
	// 예약 월이 정확한지 확인합니다.
	if (document.smsForm.chkSendFlag[1].checked && document.smsForm.R_MONTH.value.length!=2){
		alert("월이 정확하지 않습니다.");
		document.smsForm.R_MONTH.focus();
		return false;
	}

	// 에약 일이 정확한지 확인합니다.
	if (document.smsForm.chkSendFlag[1].checked && document.smsForm.R_DAY.value.length!=2){
		alert("일이 정확하지 않습니다.");
		document.smsForm.R_DAY.focus();
		return false;
	}

    for (k=1; k < document.smsForm.strDest.length; k++) { 
        document.smsForm.strTelList.value += document.smsForm.strDest.options[k].value;
