...
<table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed">
	<tr height="25px">
		...
		<td width="90" align="center" bgcolor="#E8ECF1"><strong>팀</strong></td>
		<td style="padding-left:10px;">
		<?=($vcfg['use_category'] && $_category_name_array[$cat_num]!='')?"[".$_category_name_array[$cat_num]."] ":""?> <?=$bd_name?>
		</td>
	</tr>
	<tr>
		<td colspan="4" height="1" bgcolor="#CCCCCC"></td>
	</tr>
	<tr height="25px" style="padding-top:5px;">
		<td width="90" align="center" bgcolor="#E8ECF1"><strong>매장</strong></td>
		<td style="padding-left:10px;">
		<?=$bd_subject?>
		</td>
		<td width="90" align="center" bgcolor="#E8ECF1"><strong>업체</strong></td>
		<td style="padding-left:10px;">
		<?=$bd_bus?>
		</td>
	</tr>
...
<span style="font-size:15px;font-color:#9d9d9d;"><?=$bd_content?></span>
</div>
...
<div>
	<?php  
			...
			$query="select * from wb_tb_bbs_body where bd_subject='$bd_subject' and bd_ext5 < '$bd_ext5' order by bd_ext5 asc limit 3";
			$result = mysql_query($query);
			
			while($rows = @mysql_fetch_assoc($result)){
			$time_arr = explode('-',date('Y-m-d',$bd_ext5));
			$z++;
			
	?>
			<?if($z<2){echo "최근일정:";}?><?if($z==1){$sld="display:none;";} else if($z==2){$sld="display:inline;";}?><span style="<?=$sld?>"> / </span><a href="http://115.68.22.75/croco/wb_board/view.php?&bbs_code=croco_calendar&bd_num=<?=$rows[bd_num]?>&year=<?=$time_arr[0]?>&month=<?=$time_arr[1]?>"><?php echo date('Y-m-d',$rows['bd_ext5']); ?></a>  
	<?php }?>
</div>
...
<br />
<script>
//location.href='<?=$url_modify?>' 수정
// delchk('<?=$bd_num?>') 삭제
function levelC(page){
	var mb_level ="<?=$_mb['mb_level']?>";
	var employees_level ="<?=$_const['employees_level']?>";
	if(mb_level<employees_level){
...
<table width="100%" border="0">
	<tr>
		<td align="left">
<? if($vcfg['btn_modify']) { ?>	
<img src="<?=$skin_url?>images/modify.gif" onClick="levelC('write')" style="cursor:pointer" align="absmiddle">
<? } ?>
<? if($vcfg['btn_del']) { ?>	
<img src="<?=$skin_url?>images/delete.gif" style="cursor:pointer" align="absmiddle" onclick="levelC('del')">
<? } ?>
<? if($vcfg['btn_reply']) { ?>	
...
<? } ?>
<script>
function delchk(bd_num){
	if(confirm("정말 삭제하시겠습니까?")){
		location.href='write.php?bd_num='+bd_num+'&mode1=deleteone&bbs_code=croco_calendar&month=<?=$month?>';
	};
