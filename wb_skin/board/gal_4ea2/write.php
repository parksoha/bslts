...
<div id="g_write_style" class="board_write_style_pc">
<form name="write_form" method="post" action="?<?=$_get_param[3]?>" onSubmit="return validate(this)" enctype="multipart/form-data">
	...
	<h1><? if($mode=='write') echo "새글쓰기"; elseif($mode=='modify') echo "글수정"; else echo "답변글 등록";?></h1>
	<? } ?>
	<div class="input_list">
		<ul class="list">
			...
			<? if($wcfg['input_name']) { ?>
			<li>
				<span class="title need">작성자</span>
				<input name="bd_name" type="text" value="admin" class="input">
			</li>
			<?}?>
			<? if($wcfg['input_pass']) { ?>
			<li>
				<span class="title need">암호</span>
			...
			<li>
				...
				<?if($mode =='reply'){?>
				<input name="bd_email" type="text" class="input" value="<?=$_site_info['admin_email']?>" size="40">			
				<?}else{?>
			...
			<? if($wcfg['use_home']) { ?>
			<li>
				<span class="title">상단모델명</span>
				<input name="bd_home" type="text" class="input" value="<?=$bd_home?>" size="60">
			</li>
			<?}?>
			<li>
				<span class="title need">모델명</span>
				<input name="bd_subject" type="text" class="input" value="<?=$bd_subject?>" size="60" maxlength="200">
			</li>

			<!--<li>
				<span class="title need">제조사</span>
				<input name="bd_subject2" type="text" class="input" value="<?=$bd_subject2?>" size="60" maxlength="200">
			</li>

			<li>
				<span class="title need">가격</span>
				<input name="bd_subject3" id="pric_p" type="text" class="input" value="<?=$bd_subject3?>" size="60" maxlength="200"> <label><input name="pric_pq" type="checkbox" onclick="pric_ps();">가격협의</label>
			</li>-->
			<script>
			function pric_ps(){
				
				var animal = $('input:checkbox[name=pric_pq]').is(':checked');
				if(animal == false){
					$("#pric_p").val("<?=$bd_subject3?>");
				}else{
					$("#pric_p").val("가격협의");
			...
			<li>
				...
				<span class="input_label">이름 : </span><input name="bd_links[<?=$i?>][name]" type="text" class="input" value="<?=$bd_links[$i]['name']?>" size="60"><br />
				<span class="input_label">URL : </span><input name="bd_links[<?=$i?>][url]" type="text" class="input" value="<?=$bd_links[$i]['url']?>" size="60">
			</li>
			<?
				}
			}?>
			...
			<li>
				...
				<? if($bd_files[$i][name]!='') { ?>
					<?=$bd_files[$i][name]?>
					<input type="checkbox" name="bd_files_del[<?=$i?>]" value="1" /> 삭제
				<? } ?>
			</li>
			<?
				}
	...
	<div class="btns">
		<?
			switch($mode) {
				case 'write' :
		?><input type="image" src="<?=$skin_url?>images/confirm.gif" onclick="submitContents();" style="cursor:pointer" align="absmiddle"><?
				break;
				case 'reply' :
		?><input type="image" src="<?=$skin_url?>images/confirm.gif" onclick="submitContents();" style="cursor:pointer" align="absmiddle"><?
				break;
...
<div id="g_write_style" class="board_write_style_mo">
<form name="write_form" method="post" action="?<?=$_get_param[3]?>" onSubmit="return validate(this)" enctype="multipart/form-data">
	<input type="hidden" name="mode" value="<?=$mode?>">
	<input type="hidden" name="act" value="ok">
	<input type="hidden" name="bd_num" value="<?=$bd_num?>">
	...
	<ul class="input_list">
		<? if($wcfg['use_category']) { ?>
		<li>
			<span class="title need">분류</span>
			<select name="cat_num" hname="분류" required>
				<option value="">+선택+</option>
				<?=rg_html_option($_category_info,$cat_num,'cat_num','cat_name')?>
			</select>
		</li>
		...
		<? } ?>
		<li>
			<span class="title need">제목</span>
			<input name="bd_subject" type="text" class="input" value="<?=$bd_subject?>" hname="제목" required placeholder="제목" />
		</li>
		<li class="inc_textarea">
			<span class="title need">요약내용</span>
			<textarea name="bd_ext1" hname="요약내용" required placeholder="요약내용" class="text_area_short"/><?=$bd_ext1?></textarea>
		</li>
		<? if($wcfg['use_home']) { ?>
		<li>
			<span class="title">홈페이지</span>
			<input name="bd_home" type="text" value="<?=$bd_home?>" placeholder="홈페이지 : ex) http://yahoo.com" /> 
		</li>
		<? } ?>
		...
		?>
		<li>
			<span class="title need">상세내용</span>
		...
		<li>
			<span class="title">링크 #<?=($i+1)?></span>
			<span class="input_label">이름 : </span><input name="bd_links[<?=$i?>][name]" type="text" class="input" value="<?=$bd_links[$i]['name']?>" size="55"><br />
			<span class="input_label">URL : </span><input name="bd_links[<?=$i?>][url]" type="text" class="input" value="<?=$bd_links[$i]['url']?>" size="55">
		</li>
		<?
			}
		}
		...
		?>
		<li>
			<span class="title"><?=$upload_name?> #<?=($i+1)?></span>
			<input type="file" name="bd_files[<?=$i?>]" class="input" size="50">
