...
<div id="board_view_style" class="board_view_style_pc" style="width:100%; float:left;">
	...
	<div class="article_info">
		...
		<ul class="info_list">
			...
			<li>Writer : <span onClick="rg_bbs_layer('<?=$bbs_code?>','<?=$bd_num?>','<?=$bd_name?>','<?=$mb_id?>','<?=$open_homepage?>','<?=$open_email?>','<?=$open_profile?>','<?=$open_memo?>')" style='cursor:pointer;'><?=($mb_icon)?$mb_icon:$bd_name?></span>
			</li>
	...
	<? if($bd_ext1) { ?>
	<div class="tags">
		<b style="width:40px;font-size:18px;">Tag</b>
		<?
	...
	<div style="clear:both;margin-bottom:10px;padding:8px;text-align:left;background:#F0F0F0;">
		<b style="width:40px;font-size:18px;">Signature</b>
...
<div id="board_view_style" class="board_view_style_mo">
	...
	<div class="btn_group">
		<div class="left">
			<? if($vcfg['btn_modify']) { ?>
			<a href="<?=$url_modify?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">Modify</a>
			<? } ?>
			<? if($vcfg['btn_del']) { ?>
			<a href="<?=$url_delete?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">Delete</a>
			<? } ?>
			<? if($vcfg['btn_reply']) { ?>	
			<a href="<?=$url_reply?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">Apply</a>
			<? } ?>
			<? if($vcfg['vote_yes']) { ?>
			<a href="<?=$url_vote_yes?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">Agreement</a>
			<? } ?>
			<? if($vcfg['vote_no']) { ?>
			<a href="<?=$url_vote_no?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">Opposition</a>
			<? } ?>
		</div>
		<div class="right">
			<? if($vcfg['btn_list']) { ?>
				<a href="<?=$url_list?><?=($list_type)?"&list_type=".$list_type:"";?>" class="btn">List</a>
			<? } ?>
		</div>
	</div>
</div>
	...
	function changesize(flag){
		...
		if(!isNaN(num)){
			if(flag=='+'){
				...
				alert('It is already the minimum size of the font.');
			}
		}
	}
