...
<form name="write_form" method="post" action="?<?=$_get_param[3]?>" onSubmit="return validate(this)" enctype="multipart/form-data">
	...
	<h1><? if($mode=='write') echo "New writing registration"; elseif($mode=='modify') echo "Revision of text"; else echo "Answer text registration";?></h1>
	<ul class="input_list">
		<? if($wcfg['use_notice'] || $wcfg['use_secret'] || $wcfg['use_reply_mail']) { ?>
		<li>
			<span class="title">Type of writing</span>
			<? if($wcfg['use_notice']) { ?><input type="checkbox" name="bd_notice" value="1" <?=$chk_bd_notice?>> Notice&nbsp;&nbsp;<? } ?>
			<? if($wcfg['use_secret']) { ?><input type="checkbox" name="bd_secret" value="1" <?=$chk_bd_secret?>> Secret&nbsp;&nbsp;<? } ?>
			<? if($wcfg['use_reply_mail']) { ?><input type="checkbox" name="bd_reply_mail" value="1" <?=$chk_bd_reply_mail?>> Receive answer mail&nbsp;&nbsp;<? } ?>
		</li>
		<? } ?>
		<? if($wcfg['input_name']) { ?>
		<li>
		...
		?>
		<li>
			<span class="title">Link <?=($i+1)?></span>
			Nmae :<input name="bd_links[<?=$i?>][name]" type="text" class="underline" value="<?=$bd_links[$i]['name']?>" ><br>
		...
		?>
		<li>
			<span class="title">The attached file <?=($i+1)?></span>
		...
		<? if($wcfg['spam_chk']) { ?>
		<li>
			<span class="title">Spam protection</span>
			<?=$spam_chk_img?> Please enter the text on the left.
...
<form name="write_form" method="post" action="?<?=$_get_param[3]?>" onSubmit="return validate(this)" enctype="multipart/form-data">
	...
	<div class="write_top">
		...
		<div class="btn_group">
			<div class="left">
				<a href="javascript:history.back();">Cancle</a>
			</div>
			<div class="right">
				<input type="submit" class="btn" value="<?
				switch($mode) {
					case 'write' : echo "save"; break;
					case 'reply' : echo "reply"; break;
					case 'modify' : echo "modify"; break;
				}
				?>" />
			</div>
		</div>
	...
	<ul class="input_list">
		...
		<li>
			<span class="title">Type of writing</span>
			<? if($wcfg['use_notice']) { ?><input type="checkbox" name="bd_notice" value="1" <?=$chk_bd_notice?>> Notice&nbsp;&nbsp;<? } ?>
			<? if($wcfg['use_secret']) { ?><input type="checkbox" name="bd_secret" value="1" <?=$chk_bd_secret?>> Password&nbsp;&nbsp;<? } ?>
			<? if($wcfg['use_reply_mail']) { ?><input type="checkbox" name="bd_reply_mail" value="1" <?=$chk_bd_reply_mail?>> Receive answer mail&nbsp;&nbsp;<? } ?>
		</li>
		<? } ?>
		<? if($wcfg['input_name']) { ?>
		...
		<li>
			<span class="title">Link <?=($i+1)?></span>
			Name :<input name="bd_links[<?=$i?>][name]" type="text" class="underline" value="<?=$bd_links[$i]['name']?>" ><br>
			URL :<input name="bd_links[<?=$i?>][url]" type="text" class="underline" value="<?=$bd_links[$i]['url']?>" size="60" style="color:blue; text-decoration:underline;">
		</li>
		<?
			}
