...
<div id="board_style">
	...
	<div class="search_box search_box_mo" style="border:0; width:300px; margin:0 auto; padding-top:10px; padding-right:0;">
		...
		<?=$_post_param[0]?>
			<div class="hide_box">
				<? if($ss['cat']) { ?>
				<label><input type="checkbox" name="ss[cat]" value="<?=$ss['cat']?>" checked>Search within classification</label>
				<? } ?>
				<!--<label><input type="checkbox" name="ss[si]" value="1" checked>아이디</label>-->
				<label><input type="checkbox" name="ss[sn]" value="1" checked>Writer</label>
				<label><input type="checkbox" name="ss[st]" value="1" checked>Title</label>
				<label><input type="checkbox" name="ss[sc]" value="1" checked>Contents</label>
			</div>
			<input name="kw" type="text" id="kw" class="btn" value="<?=$kw?>" hname="search word"  style="border:1px solid #ccc; width:255px; border-radius:5px;"/>
			<input type="submit" value="Search" style="position:inherit;"/> 
...
<?if($_bbs_auth['write']||$_bbs_auth['admin']){?>	
	<div class="btn_group btn_group_mo" style="margin-top:15px;">
		...
		<div class="left">
			...
			<a href="javascript:board_manager()">admin</a>
		...
		<div class="right">
			...
			<a class="btn strong" href="write.php?<?=$_get_param[3]?><?=($list_type)?"&list_type=".$list_type:"";?>">write</a>
	...
	<div class="search_box search_box_pc">
		...
		<?=$_post_param[0]?>
			...
			<div class="hide_box">
				...
				<label><input type="checkbox" name="ss[sn]" value="1" <?=$checked_sn?>>Writer</label>
				<label><input type="checkbox" name="ss[st]" value="1" <?=$checked_st?>>Title</label>
				<label><input type="checkbox" name="ss[sc]" value="1" <?=$checked_sc?>>Contents</label>
			</div>


			<input name="kw" type="text" id="kw" value="<?=$kw?>" hname="search word" style="border:0px;width:150px;height:24px;color:#666666; border:1px solid #CCCCCC; background:#ffffff;" required>
