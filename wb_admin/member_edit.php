<?
	...
	if($dels=='1'){
	$chkcnt= count($chk_nums);

		for($y=0; $y<$chkcnt; $y++){

			mysql_query("delete from wb_tb_member where mb_num='".$chk_nums[$y]."'");
		}

	rg_href("member_list.php");
	}
	...
	if($_SERVER['REQUEST_METHOD']=='POST') {
		...
		if($mb_pass!='') $mb_pass = get_password_str($mb_pass);
		if($mb_jumin!='') $mb_jumin = get_password_str($mb_jumin);
...
//		if($validate->is_empty($mb_nick)) $mb_nick=$mb_id;
		...
		// 파일 업로드
		$mb_files=upload_file($_path['member_data'],"mb_files",$num,$data['mb_files'],$mb_files_del);
	...
	$c_mb_is_sms[$data['mb_is_sms']]='checked';
...
<table border="1" cellpadding="3" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#E1E1E1">
	...
	<tr>
		<td width="120" align="left" bgcolor="#F0F0F4"><strong>회원구분</strong></td>
		<td>
		<label><input name="mb_licen" type="radio" value="1" <?if($data['mb_licen'] == "1"){echo "checked";}?> <?=$chkeds?>> 개인회원</label>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<label><input name="mb_licen" type="radio" value="2" <?if($data['mb_licen'] == "2"){echo "checked";}?>> 사업자회원</label>
	...
	<tr>
		<td width="120" align="left" bgcolor="#F0F0F4"><strong>상호명</strong></td>
		<td>
		<input name="mb_busin" type="text" value="<?=$data['mb_busin']?>" class="input">
	...
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>서업자번호</strong></td>
		<td><input name="mb_businnum1" type="text" size="4" value="<?=$businnum_row[0]?>" class="input">
		-
		<input name="mb_businnum2" type="text" size="4" value="<?=$businnum_row[1]?>" class="input">
		-
		<input name="mb_businnum3" type="text" size="10" value="<?=$businnum_row[2]?>" class="input">
		</td>
	</tr>
	...
	<tr>
		<td width="120" align="left" bgcolor="#F0F0F4"><strong>아이디</strong></td>
		<td><input name="mb_id" type="text" value="<?=$data['mb_id']?>" class="input">
&nbsp;<input type="button" onClick="search_mb_id2('<?=$_url['member']?>','member_form|mb_id',document.member_form.mb_id.value)" value="중복확인" class="button"></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>암호</strong></td>
		<td><input name="mb_pass" type="password" value="" class="input" hname="암호" match="mb_pass1">
		중복확인 <input name="mb_pass1" type="password" value="" class="input">
		[변경할 경우 입력하세요.]</td>
	</tr>
<? if($member_form['mb_name']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>회사(이름)<span style="color:red;">v</span></strong></td>
...
<? if($member_form['mb_tel2']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>핸드폰번호<span style="color:red;">v</span></strong></td>
...
<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>레벨<span style="color:red;">v</span></strong></td>
...
<? if($member_form['mb_nick']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>닉네임</strong></td>
		<td><input name="mb_nick" type="text" value="<?=$data['mb_nick']?>" class="input"></td>
	</tr>
<? } // end if mb_nick ?>
<? if($member_form['mb_email']!=0) { ?>  
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>이메일</strong></td>
...
<? if($member_form['mb_tel1']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>전화번호</strong></td>
...
<? if($member_form['mb_address']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>우편번호</strong></td>
		<td><input name="mb_post1" type="text" size="10" value="<?=$data[mb_post]?>" class="input">

<input type="button" onClick="javascript:openDaumPostcode();" value="우편번호" class="button">
<input type="checkbox" name="of[mb_post]" value="1" <?=$c_of['mb_post']?> />
주소공개</td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>주소</strong></td>
...
<? if($member_form['mb_signature']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>서명</strong></td>
...
<? if($member_form['mb_introduce']!=0) { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>자기소개</strong></td>
...
<? } // end if mb_introduce ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>관리자메모</strong></td>
		<td><textarea name="mb_admin_memo" cols="60" rows="3"><?=$data['mb_admin_memo']?></textarea></td>
	</tr>
<? if($member_form['icon1']!=0) { ?>
	<tr>
    <td align="left" bgcolor="#F0F0F4"><strong>아이콘</strong></td>
...
<? if($member_form['photo1']!=0) { ?>
	<tr>
    <td align="left" bgcolor="#F0F0F4"><strong>사진</strong></td>
...
<? } // end if photo1 ?>
	<tr>
	  <td align="left" bgcolor="#F0F0F4">&nbsp;</td>
	  <td><input name="mb_is_mailing" type="checkbox" id="mb_is_mailing" value="1" <?=$c_mb_is_mailing[1]?> checked>
      <strong>메일수신</strong> &nbsp;
	  <input name="mb_is_sms" type="checkbox" id="mb_is_sms" value="1" <?=$c_mb_is_sms[1]?> checked>
      <strong>SMS수신</strong> &nbsp;
      <input name="mb_is_opening" type="checkbox" id="mb_is_opening" value="1" <?=$c_mb_is_opening[1]?>>
      <strong>정보공개</strong></td>
	  </tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>상태</strong></td>
...
<? if($mode=='modify') { ?>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>가입일</strong></td>
		<td><?=rg_date($data['join_date'])?></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>가입IP</strong></td>
		<td><?=$data['join_ip']?></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>로그인</strong></td>
		<td><?=rg_date($data['login_date'])?></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>로그인IP</strong></td>
		<td><?=$data['login_ip']?><br></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>로그인회수</strong></td>
		<td><?=$data['login_count']?><br></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>최근정보수정일</strong></td>
		<td><?=rg_date($data['modify_date'])?></td>
	</tr>
	<tr>
		<td align="left" bgcolor="#F0F0F4"><strong>정보수정IP</strong></td>
