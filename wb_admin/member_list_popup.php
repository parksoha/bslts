				...
				case '0' : 
					if($kw!='' && $__v!='') {
						...
						switch ($__v) {
							...
							case '3' : $jumin=$dbcon->escape_string(get_password_str($kw));
...
<script>
	...
	{	
		...
		<? if($f_value!='') { ?>
		if(<?=$f_value?>.value=='')
			<?=$f_value?>.value=rtn_value;
		else
			<?=$f_value?>.value=<?=$f_value?>.value+','+rtn_value;
		<? } ?>

		<? if($f_text!='') { ?>
		if(<?=$f_text?>.value=='')
			<?=$f_text?>.value=rtn_text;
		else
			<?=$f_text?>.value=<?=$f_text?>.value+','+rtn_text;
		<? } ?>
