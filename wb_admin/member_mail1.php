...
if($id=="1" && $re != "1"){
	...
	if($Idx_Ary != 'all'){
		if($num_ord=="1"){
			if(strlen($Idx_Ary)>1) $Idx_Ary = str_replace('","',"','",$Idx_Ary);
			$rs_list->add_where("mb_num in (".$Idx_Ary.") group by mb_email");
	...
	if($Idx_Ary2 != 'all'){
		if($num_ord=="1"){
			...
			$rs_list->add_where("mb_num in (".$Idx_Ary2.") group by mb_email");
	...
	if($Idx_Ary3 != 'all'){
		...
		$rs_list->add_where("mb_num in (".$Idx_Ary3.")");
	...
	if($Idx_Ary3 != 'all'){
		if($num_ord=="1"){
			if(strlen($Idx_Ary3)>1) $Idx_Ary3 = str_replace('","',"','",$Idx_Ary3);			
			$rs_list->add_where("mb_num in (".$Idx_Ary3.") group by mb_email");
...
while($R=$rs_list->fetch()) {	
	...
	if($R[mb_email] != ''){
		$add_ary[mb_email][] = $R[mb_email]."/".$R[mb_name];
...
if($_SERVER['REQUEST_METHOD']=='POST' && $mode=='send'){
		...
		for($i=0;$i<count($add_ary[mb_email]);$i++){

			


			
			$mail_compan = explode("/",$add_ary[mb_email][$i]);
		...
		if($email_ok == "ok") {
			...
			$email_hap = count($add_ary[mb_email]);

			if(count($email_hap > 1)){
				$ids_i = $add_ary[email1][0]."외 ".$email_hap."명";

			}else{
				$ids_i = $add_ary[mb_email][0];
			}
...
$mail_chks = implode("/",$add_ary[mb_email]);
$chk_count = count($add_ary[mb_email]);
...
<div style="width:100%; float:left; margin-top:20px;">
	...
	<form method='post' name="mailform" action='member_mail1.php?id=<?=$id?>&re=1' enctype="multipart/form-data">
...
function autotext(ok){
	...
	form.action = 'member_mail1.php?id=<?=$id?>&re=1&auto='+ok;
	form.submit();
}
</script>
