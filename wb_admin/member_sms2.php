...
$rs_list->clear();
$rs_list->set_table('wb_tb_free_form');
if($id=="1" && $re == ""){
	if($Idx_Ary != 'all'){
	...
	if($Idx_Ary != 'all'){
		if($num_ord=="1"){
			$rs_list->add_where("free_form_num in ('".$Idx_Ary."') group by free_form_hpno");
		}else{
			$rs_list->add_where("free_form_num in ('".$Idx_Ary."')");
		}
...
while($R=$rs_list->fetch()) {
	if(strlen($R[free_form_hpno])<5) continue;
	$hp = str_replace("-","",$R[free_form_hpno]);
	$add_ary[hp][] = $hp;
	$add_ary[name][] = $R[free_form_name];/**/
}
...
<div style="width:618px; height:877px; float:left; background:url('images/phone2.png') center no-repeat;">
...
<div style="width:200px; height:auto; float:left; background-color:#efefef; text-align:center; padding:10px 0 10px 0; margin:20px 0 0 20px;">
        ...
        <div style="margin:auto; background-image:url('<?=$_path['sms']?>images/smsbg.gif'); width:160px; height:120px; margin-top:8px;">
            <textarea name='strData' id='sms_message' style="font-family:굴림체; color:#000; line-height:15px;margin:auto; margin-top:20px; overflow: hidden; width:140px; height:88px; font-size: 9pt; border:0; background-color:#88C8F8;" cols="16" onchange="ChkLen()" onkeyup="ChkLen()"><?=$_GET['content']?></textarea>
        </div>
        <div style="text-align:center; margin:5px 0 5px 0;">
            <input type="text" name="strDataCount" size="3" style="border:0; background-color:#F8F8F8; text-align:right;" maxlength="3" value="0" readonly> / <div id="maxLength" style="display:inline;">90</div> byte
...
<form name="smsForm2" method="post" action="member_sms2.php?id=<?=$id?>&re=1" style="padding:0; margin:0;">

<input type="hidden" value="<?=$Idx_Ary?>" name="Idx_Ary">

<div style="width:200px; height:auto; float:right; margin:20px 50px 0 0; font-size:14px; text-align:center;">중복전화번호 체크<input type="checkbox" name="num_ord" value='1' <?if($num_ord=='1'){echo "checked"; }?>><input type="button" value="확인" onclick="number_j();">
