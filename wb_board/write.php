					...
					if($_mb['mb_num']!=$data['mb_num']) {
						...
						if($data['bd_pass']=='') {
							...
							if($old_pass=='') { // 입력된 암호 없다면
								...
								// 입력된 암호 있다면 비교
								if($data['bd_pass']!=get_password_str($old_pass)) {
									// 암호가 다르다면
									// 에러 메시지 표시
	...
	*/
			...
			if($data['bd_depth']=='0' && $data['mb_num']) {
				if($_bbs_info['point_write'] > 0)
					set_point($data['mb_num'],$_po_type_code['bbs'],
	...
	if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['act']=='ok') {
		...
		// 일정 게시판
		if($_bbs_info['bbs_skin'] == "calender") {
			if($lay_date) {
				$bd_ext5 = mktime(0, 0, 0, substr($lay_date,5,2), substr($lay_date,8,2), substr($lay_date,0,4));
			}
			if(is_array($_POST[bd_subject])){
				for($a=0; $a < 12; $a++) {
					if($_POST[bd_subject][$a] == "") {
						if($a == 12-1) {
							$_POST[bd_subject][$a] = "0000";
						} else {
							$_POST[bd_subject][$a] = "0000:";
						}
					} else {
						if($a == 12 - 1) {
							$_POST[bd_subject][$a] = $_POST[bd_subject][$a];
						} else {
							$_POST[bd_subject][$a] = $_POST[bd_subject][$a].":";
						}
					}
					$bd_array .= $_POST[bd_subject][$a];
				}
				$bd_subject = $bd_array;
			}
		}
		...
		if($bbs_code=="03_board1") { $wcfg['use_html'] = "1"; $bd_html = "1"; }
    $bd_html = ($wcfg['use_html'])?$bd_html:'0';
		$bd_home = ($wcfg['use_home'])?$bd_home:'';
		...
		if($bd_pass!='')
			$rs_write->add_field("bd_pass",get_password_str($bd_pass));
...
/*				$tmp_pol_data=serialize(array(
				...
				set_point($mb_id,$_bbs_info['point_write'],0,'글쓰기 포인트 추가',$_bbs_info['bbs_name'],$tmp_pol_data);
*/
			break;
			case 'reply' : // 응답글
		...
		// 파일 업로드
		if($wcfg['use_upload']) {
			$bd_files=upload_file($_path['bbs_data'],"bd_files",$bd_num,$data['bd_files'],$bd_files_del);
		...
		if( $mode!='modify' && file_exists($skin_path."mail.php") && (
			...
			$mail_content = rg_conv_text($bd_content,$bd_html);
