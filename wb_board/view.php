...
//	$vcfg['view_signature']=($_view_cfg['view_signature'] <= $tmp_level);
	...
	if($mode=='comment_write' || $mode=='comment_delete') {
		if($mode=='comment_write') {
			...
			$rs->add_field("bc_depth","$bc_depth");
			$rs->add_field("bc_parent_num","$bc_parent_num");
			...
			$rs->add_field("bc_pass",get_password_str($bc_pass));
			$rs->add_field("bc_email","$bc_email");
			$rs->add_field("bc_home","$bc_home");
			...
			if($_bbs_info['point_comment'] > 0 && $_mb)
				set_point($_mb['mb_num'],$_po_type_code['bbs'],
	...
	*/
					...
					if($_mb['mb_num']==$data_comment['mb_num']) {
						...
						if($data_comment['bc_pass']=='') {
							...
							if($old_pass=='') { // 입력된 암호 없다면
								...
								// 입력된 암호 있다면 비교
								if($data_comment['bc_pass']!=get_password_str($old_pass)) {
									// 암호가 다르다면
									// 에러 메시지 표시
			...
			$comment_dept ="select bc_depth from wb_tb_bbs_comment  where bc_num ='".$bc_num."'";
			$comment_dept = mysql_query($comment_dept);
			$comment_dept = mysql_fetch_array($comment_dept);
			if($comment_dept["bc_depth"]=="0"){
				$rs->clear();
				$rs->set_table($_table['bbs_comment']);
				$rs->add_where("bd_num=$bd_num");
				$rs->add_where("bc_parent_num=$bc_num");
				$rs->delete();
			}
			...
			if($_bbs_info['point_comment'] > 0 && $data_comment['mb_num'])
				set_point($data_comment['mb_num'],$_po_type_code['bbs'],
	...
	/* 
		1. 답변글
			1-1. 로그아웃 상태
				1-1-1. 답변글에 대한 원글을 회원이 작성
					1-1-1-1. 로그인 메시지
				1-1-2. 답변글에 대한 원글을 비회원 작성
					1-1-2-1. 입력 패스워드가 없을경우 패스워드 입력창
					1-1-2-2. 입력 패스워드가 있을경우 원글 패스워드와 비교 후 메시지
			1-2. 로그인 상태
				1-2-1. 답변글에 대한 원글을 회원이 작성
					1-2-1-1. 원글 회원번호와 현재 로그인 상태의 회원번호와 비교 후 결과 메시지
				1-2-2. 답변글에 대한 원글을 비회원 작성
					1-2-2-1. 입력 패스워드가 없을경우 패스워드 입력창
					1-2-2-2. 입력 패스워드가 있을경우 원글 패스워드와 비교 후 메시지
		2. 원글
			2-1. 로그아웃 상태
				2-1-1. 회원이 작성
					2-1-1-1. 로그인 메시지
				2-1-2. 비회원 작성
					2-1-2-1. 입력 패스워드가 없을경우 패스워드 입력창
					2-1-2-2. 입력 패스워드가 있을경우 현재글 패스워드와 비교 후 메시지
			2-2. 로그인 상태
				2-2-1. 회원이 작성
					2-2-1-1. 현재글 회원번호와 현재 로그인 상태의 회원번호와 비교 후 결과 메시지
				2-2-2. 비회원 작성
					2-2-2-1. 입력 패스워드가 없을경우 패스워드 입력창
					2-2-2-2. 입력 패스워드가 있을경우 원글 패스워드와 비교 후 메시지
	*/
	    
	if($bd_secret > 0) {
		if(!($_auth['bbs_admin'] || $_bbs_auth['secret'])) {        // 권한 없으면
			
			if($bd_parent_num) { // 답변글
				$rs->clear();
				$rs->set_table($_table['bbs_body']);
				$rs->add_where("bd_num=$bd_parent_num");
				$data1=$rs->fetch();
				if(!$data1) {
					rg_href("list.php?$_get_param[3]");
					exit;
				}
				$bd_pass = $data1['bd_pass']; // 원글의 패스워드

				if(!$_mb) { // 로그아웃 상태
					if($data1['mb_num']) { // 원글이 회원이 작성한 글 
						$_msg_type='view_secret_error_auth_guest';
						include("msg.php"); // 로그인 메시지
					} else { // 원글이 비회원이 작성한 글
						if($old_pass == "") {
							...
							if($bd_pass!=get_password_str($old_pass)) { // 원글 패스워드와 입력한 패스워드와 비교
					...
					if($data1['mb_num']) { // 원글이 회원이 작성한 글
						if($data1['mb_num'] != $_mb['mb_num']) { // 원글 회원번호와 현재 로그인 상태의 회원번호와 비교
						...
						if($old_pass == "") {
							...
							if($bd_pass!=get_password_str($old_pass)) {
								$_msg_type='view_secret_pass_error';
								include("msg.php");
								exit;
							}
	...
	if($mb_num) { // 회원이 쓴 글
		...
		$mb_data=$rs->fetch();
		if($mb_data && $mb_data['mb_is_opening'] && $_mb) {
			$mb_of=unserialize($mb_data['mb_open_field']);
			$open_profile='1';
			$open_memo='1';
		}
		if($mb_data) $mb_data['mb_files']=unserialize($mb_data['mb_files']);

		if(($_list_cfg['use_mb_icon'] <= $mb_data[mb_level]) && ($mb_data['mb_files']['icon1']['name']!='')){
			if(file_type_chk($mb_data['mb_files']['icon1']['type'],'image')) {
				$icon_data = rg_base64("mb_num=$mb_num&key=icon1");
				$mb_icon = "<img src=\"{$_url['member']}mb_data.php?mb_data=$icon_data\" align=absbottom>";
				unset($icon_data);
			}
			unset($is_image);
		}
	...
	if($bd_home!='') $open_homepage= base64_encode($bd_home) ; // 홈페이지
	if($bd_email!='') $open_email= base64_encode($bd_email); // 이메일
...
//  write 에서 변환해서 저장됨
//	if(!$is_admin && ($bd_html == 1 || $bd_html == 2)) {
